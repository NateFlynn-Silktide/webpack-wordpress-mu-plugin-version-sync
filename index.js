...
module.exports = class WordpressWebpackMUPluginVersionSync {
    ...
    constructor( options = {} ) {
        this.options = { ...WordpressWebpackMUPluginVersionSync.defaultOptions, ...options };
    }

    apply( compiler ) {
        const pluginName = WordpressWebpackMUPluginVersionSync.name;
        ...
        if( ! typeof this.options.outputFile == 'array' ) throw new Error( `Error: ${pluginName} requires \`outputFile\` to be an array containing at least 2 output file paths in order to write synced content.` );
        ...
        compiler.hooks.afterEmit.tapAsync( pluginName, ( compilation, callback ) => {
            ...
            for( var outputFile of this.options.outputFile ) {
                // console.log( outputFile );

                // Read the current outputFile content
                const outputFileContents = fs.readFileSync( outputFile, { encoding: 'utf-8' } );
                const search = /\/\*\*([\s\S]*?)\*\//;
            ...
            // Log write success
            console.log( `${config['Plugin Name']}: File Headers Updated` );
